...
class TypeDossierSignatureEtape implements TypeDossierEtapeSetSpecificInformation {
	...
	public function setSpecificInformation(TypeDossierEtape $typeDossierEtape,array $result) : array{
        ...
        if (empty($result['action']['verif-iparapheur']['connecteur-type-mapping'])) {
            $result['action']['verif-iparapheur']['connecteur-type-mapping'] = [];
        }
		...
		if (empty($typeDossierEtape->specific_type_info['has-date-limite'])){
            ...
            $result['action']['verif-iparapheur']['connecteur-type-mapping']['iparapheur_has_date_limite'] = 'has_date_limite';
            $result['action']['verif-iparapheur']['connecteur-type-mapping']['iparapheur_date_limite'] = 'date_limite';
		}
		...
		foreach(['objet'=>'libelle_parapheur','document'=>'document_a_signer','autre_document_attache'=>'annexe'] as $mapping_key => $specific_key) {
			if (!empty($typeDossierEtape->specific_type_info[$specific_key])) {
                ...
                $result['action']['verif-iparapheur']['connecteur-type-mapping'][$mapping_key] = $typeDossierEtape->specific_type_info[$specific_key];
			}
		}
		return $result;
	}
