...
class RecuperationFichierLocal extends RecuperationFichier {
	...
	public function deleteFile($filename){
		if (! unlink($this->directory."$filename")) {
			throw new UnrecoverableException("La suppression de ".$this->directory."$filename"." n'a pas été possible");
		}
		return true;
	}
	...
	public function sendFile($source_directory, $filename){
		if ($this->directory_send) {
			if (! copy($source_directory."/".$filename, $this->directory_send."/$filename")) {
				throw new UnrecoverableException("La copie de ".$source_directory."/".$filename." vers ".$this->directory_send."/$filename"." n'a pas été possible");
			}
		}
		return true;
	}
