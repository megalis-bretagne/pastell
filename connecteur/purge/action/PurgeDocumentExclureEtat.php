...
class PurgeDocumentExclureEtat extends ChoiceActionExecutor
{
    /**
     * @return bool
     * @throws Exception
    ...
    public function go()
    {
        $document_etat = $this->getRecuperateur()->get('document_exclure_etat');
        $list_etat = $this->displayAPI();
        if (empty($list_etat[$document_etat])) {
            throw new Exception("Cet état n'existe pas");
        }
        $this->getConnecteurProperties()->setData('document_exclure_etat', $document_etat);
    ...
    {
        $this->setViewParameter('document_exclure_etat', $this->getConnecteurProperties()->get('document_exclure_etat'));

        $this->setViewParameter('list_etat', $this->displayAPI());
        $this->renderPage(
            "Choix de l'état du dossier à exclure",
