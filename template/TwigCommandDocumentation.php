...
<table class="table table-striped" aria-label="Exemple de commande twig">
    ...
    <tr>
        ...
        <td>Extrait le contenu de la seconde colonne de la treizième ligne du fichier CSV identifié par l'élément data_csv (les index commencent à 0). Le contenu du fichier CSV doit être encodé en UTF-8</td>
    </tr>
    <tr>
        <td>Expression csv (avec un autre séparateur de champs)</td>
        <td> {{ csvpath('data_csv',1,12,";",'"',"\\") }}</td>
        <td>Idem que l'expression précédente, mais en spécifiant <strong>;</strong> comme caractère séparateur de colonne (courant avec un tableur en français), " comme clôture de champs et \ comme caractère d'échappement
