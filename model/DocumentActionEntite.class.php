...
class DocumentActionEntite extends SQL
{
    ...
    public function getLastActionNotModif($id_e, $id_d)
    {
        $sql = "SELECT action FROM document_action_entite " .
            " JOIN document_action ON document_action_entite.id_a = document_action.id_a " .
            " WHERE document_action_entite.id_e = ? AND id_d=? AND action != ? " .
    ...
    public function getListBySearch($id_e, $type, $offset, $limit, $search, $state, $last_state_begin, $last_state_end, $tri, $allDroitEntite, $etatTransit, $state_begin, $state_end, $notEtatTransit, $indexedFieldValue = [], $sens_tri = 'DESC')
    {
        ...
        if (! in_array($tri, array_keys($indexedFieldValue))) {
            switch ($tri) {
                case 'entite':
                    $tri = "entite.denomination $sens_tri";
                    break;
                case 'titre':
                case 'title':
                    $tri = "document.titre $sens_tri,document.id_d $sens_tri";
                    break;
                default:
                    $tri =  "document_entite.last_action_date $sens_tri";
            }
